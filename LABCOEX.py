...
logger = logging.getLogger('lens-LABCOEX')
...
logging.basicConfig(level=logging.INFO,
					...
					filename='/home/ubuntu/LABCOEX.log',
					filemode='w')
'''
...
def get_size(start_path = '/home/ubuntu/tmp_labcoex/'):
	total_size = 0
...
def cleanup():
	import shutil
	logger.info("ls /home/ubuntu/tmp_labcoex")
	try:
		ret = subprocess.check_output(["ls", "-alR", "/home/ubuntu/tmp_labcoex"], stderr=subprocess.STDOUT)
		logger.info(ret)
	...
	logger.info("clean up /home/ubuntu/tmp_labcoex")
	for root, dirs, files in os.walk('/home/ubuntu/tmp_labcoex/'):
		for f in files:
...
while True:
	...
	for index,row in field_db.iterrows():
		...
		logger.info(row[0])
		logger.info(row[1])
		if str(row[0]).find("LABCOEX")!=-1 and str(row[0]).endswith(".zip"):
			logpath=row[0]
			s3dir = os.path.dirname(logpath)
			ph = s3dir.split('/')
			if (len(ph) < 3):
				logger.info("wrong s3dir %s" % s3dir)
				ErrMessage='LABCOEX log S3 dir is wrong'
				Okfile=0
			filename = os.path.basename(logpath)
			fn = filename.split('_')
			if (len(fn) <3 ):
				logger.info("wrong file %s" % filename)
				ErrMessage='labcoex zip file name is wrong'
				Okfile=0
			if Okfile==1:
	...
	while(k<LogNum):
		try:
			...
			tmpdir= '/home/ubuntu/tmp_labcoex'
			...
			for eachfile in srczip.namelist():
				...
				if eachfile.endswith('.csv'):
					...
					with open(eachfilename,'r') as csvfile:
						reader = csv.DictReader(csvfile)
						MajorTech_lut_cn = [row['MajorTech'] for row in reader]
						logger.info(MajorTech_lut_cn)
						MajorTech_lut_cn = list(set(MajorTech_lut_cn))
						logger.info(MajorTech_lut_cn)
					with open(eachfilename,'r') as csvfile:
						reader = csv.DictReader(csvfile)
						CombineTech_lut_cn = [row['CombineTech'] for row in reader]
						logger.info(CombineTech_lut_cn)
						CombineTech_lut_cn = list(set(CombineTech_lut_cn))
						logger.info(CombineTech_lut_cn)
					with open(eachfilename,'r') as csvfile:
						reader = csv.DictReader(csvfile)
						Profiles_lut_cn = [row['Profiles'] for row in reader]
						logger.info(Profiles_lut_cn)
						Profiles_lut_cn = list(set(Profiles_lut_cn))
						logger.info(Profiles_lut_cn)
					with open(eachfilename,'r') as csvfile:
						reader = csv.DictReader(csvfile)
						Link_lut_cn = [row['Link'] for row in reader]
						logger.info(Link_lut_cn)
						Link_lut_cn = list(set(Link_lut_cn))
						logger.info(Link_lut_cn)
					with open(eachfilename,'r') as csvfile:
						reader = csv.DictReader(csvfile)
					...
					with open(eachfilename,'r') as csvfile:
						reader = csv.DictReader(csvfile)
						fw = [row['sw_build'] for row in reader]
						logger.info(fw)
						fw = list(set(fw))
						logger.info(fw[0])
					#get testcaseid
					...
					for testitem in TestItem_lut_cn:
						for majortech in MajorTech_lut_cn:
							for combinetech in CombineTech_lut_cn:
								for profiles in Profiles_lut_cn:
									for link in Link_lut_cn:
										for band in Band_lut_cn:
											if band.find('Hz')!=-1:
												band=band.rstrip('Hz')
											db_fields = "TestItem='"+testitem+"' and MajorTech='"+majortech+"' and CombineTech ='"+combinetech+"' and Profiles ='"+profiles+"' and Link ='"+link+"' and Band='"+band+"'"
											strTestCaseID = pd.read_sql_query("SELECT TestCaseID FROM LENS.LABCOEX_LUT WHERE "+db_fields,db_conn)
											logpathfordict=testitem+'_'+majortech+'_'+combinetech+'_'+profiles+'_'+link+'_'+band
											...
											for index,row in strTestCaseID.iterrows():
												logger.info(row[0])
												ChecktestcaseinTCS = pd.read_sql_query('select distinct ExecutionStatus,TestTime,DSN,FW from LENS.'+product[k]+'_TCS where TestCaseID=\''+row[0]+'\'',db_conn)
					...
					if len(TestCaseID)<1:
						...
						processdata=2
						continue
					#get cn_name and cn_type of LEN.LABCOEX
					cn_name_db=[]
					cn_type_db=[]
					if db_conn is None:
						pass
					else:
						field_db = pd.read_sql_query("SELECT COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = 'LABCOEX'",db_conn)
						logger.info(field_db)
						for index,row in field_db.iterrows():
							logger.info(row[0])
							logger.info(row[1])
							...
							if cn_name_csv[i] not in cn_name_db:
								...
								if db_conn is None:
									...
									with db_conn.cursor() as cur:
										...
										if isfloat:
											sql = "alter table LENS.LABCOEX add `"+cn_name_csv[i]+"` DOUBLE"
										else:
											sql = "alter table LENS.LABCOEX add `"+cn_name_csv[i]+"` VARCHAR(100)"
										logger.info('SQL:'+sql)
										cur.execute(sql)
									db_conn.commit()
						i+=1
					#check new column is wrote
					logger.info("This is to track")
					cn_name_db=[]
					cn_type_db=[]
					if db_conn is None:
						pass
					...
					with open(eachfilename,'r') as csvfile:
						...
						for row in reader:
							...
							if band.find('Hz')!=-1:
								band=band.rstrip('Hz')
							logpath_dict= row['TestItem']+'_'+row['MajorTech']+'_'+row['CombineTech']+'_'+row['Profiles']+'_'+row['Link']+'_'+band
							...
							db_cmd = 'insert into LENS.LABCOEX ('+db_fields+')'+' values('+db_value+')'
			...
			#clean up /home/ubuntu/tmp_labcoex
			cleanup()
